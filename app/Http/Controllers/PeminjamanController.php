...
{
    $peminjamans = Peminjaman::with(['user', 'item', 'pengembalian'])->get();
...
{
    $peminjamans = \App\Models\Peminjaman::with(['user', 'item', 'admin'])->get();
    ...
    public function store(Request $request)
{
    ...
    if ($item->stock < $jumlah) {
        return response()->json([
            'success' => false,
            'message' => "Stok barang tidak mencukupi. Sisa stok: {$item->stock}"
        ], 400);
    }
...
public function storeBatch(Request $request)
{
    $request->validate([
        ...
        'peminjaman.*.item_id' => 'required|exists:items,id',
    ...
    foreach ($request->peminjaman as $data) {
        ...
        if ($item->stock < $jumlah) {
            return response()->json([
                'success' => false,
                'message' => "Stok barang dengan ID {$item->id} tidak mencukupi. Sisa stok: {$item->stock}"
            ], 400);
        }
    }
    ...
    public function update(Request $request, $id)
{
    ...
    if ($request->status === 'Approve') {
        ...
        $peminjaman->admin_id = Auth::id(); // Tambahkan ini
        $peminjaman->save();
    ...
    public function approve(Request $request, $id)
{
    ...
    {
        $user = $request->user();
        $peminjaman = Peminjaman::where('user_id', $user->id)->with('item')->get();

        return response()->json($peminjaman);
    }

    // Tandai barang sudah diambil oleh user
